...
We will use the terms I<condition variable> and I<CV> 
interchangeably. In both cases, we are refering to
AnyEvent condition variables ( AnyEvent::condvar ).

We will often talking about waiting for a CV to return. What
...
interface to deal with data dependencies between CVs. Here's
...
Returning a scalar or a list from a chain will return this
scalar or array through the CV of the chain. If the funds
transfer succeeded, the CV will return the new balances of
each account. Otherwise, it will return the old balances
...
Hell, you will never see a documentation on functional
programming without at least mentionning map and grep. We keep
the perl terminologies for convenience in this library.

Say you have a list of elements that you neep to map, but the
required map operations is asynchronous. Well don't worry,
C<cv_map> to the rescue! Consider the following example:

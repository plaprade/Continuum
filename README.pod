...
implement most of this library's API. Whatever you return from C<then>
will be returned from that condition variable. In the above example,
that would be C<( $a2, $b2 )>. You can also return a condition
variable from within C<then> and continue chaining calls:
...
C<then> does a bit of internal magic to retrieve the values of
the returned condition variables to make them available to the next
method call.
